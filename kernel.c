...
kernel_entry(void)
{
    __asm__ __volatile__(
        "csrw sscratch, sp\n"
        // csrw(CSR Write)RISC-VのCSRへの書き込みを行う。
        // 文法は"csrw　csr, source"。
        // ここでcsrは書き込むCSRの番号、sourceは書き込むデータ
        // sscratchレジスタはRISC-VでSupervisor modeで使用されるスクラッチレジスタとなる。
        // スクラッチレジスタは一時的にデータを保存するための目的で使用されることもある。

        "addi sp, sp, -4 * 31\n"
        // 加算をするRISC-Vのアセンブリ
        // 文法"addi rd, rs1, imm"
        // rdはDestination Register結果を格納するレジスタ,rs1はSource Register加算元 1, immはimmediateは即値。rs1レジスタの値に加算される。

        "sw ra, 4 * 0(sp)\n"
        // メモリへのデータの書き込み
        // "sw rs2, offset(rs1)"
        // レジスタrs2の値をメモリのアドレス rs1 + offsetにストアする
        ...
        "csrr a0, sscratch\n" // a0にsscratch(CSR)の値を読み取って保存する。
        "sw a0, 4 * 30(sp)\n"
        "mv a0, sp\n" // spレジスタの内容をコピーしてa0レジスタに貼り付ける
        "call handle_trap\n"
